...
import type { Inventario, Distrito } from "@/shared/types";
...
export default function InventarioPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setShowModal(false);
    setEditingItem(null);
    setFormData({ id_elemento: "", nombre_elemento: "", tipo_inventario: "", distrito_id: "" });
    fetchItems();
  };
  ...
  const handleBulkUpload = async () => {
    try {
      ...
      await fetch("/api/inventario/bulk", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      setBulkJson("");
      fetchItems();
      alert(`${items.length} elementos importados correctamente`);
    } catch (error) {
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gradient-to-r from-emerald-500 to-teal-600 text-white">
                <tr>
                  <th className="px-6 py-4 text-left text-sm font-semibold">ID Elemento</th>
              ...
              <tbody className="divide-y divide-emerald-100">
                {items.map((item) => (
                  <tr key={item.id} className="hover:bg-emerald-50 transition-colors">
                    <td className="px-6 py-4 text-sm text-gray-900">{item.id_elemento}</td>
                    <td className="px-6 py-4 text-sm text-gray-900">{item.nombre_elemento}</td>
                    <td className="px-6 py-4 text-sm text-gray-600">{item.tipo_inventario}</td>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">ID Elemento</label>
                <input
                  type="text"
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setFormData({ ...formData, nombre_elemento: e.target.value })}
                  className="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
                  required
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setFormData({ ...formData, tipo_inventario: e.target.value })}
                  className="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
                  required
                />
              </div>
      ...
      {showBulkUpload && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full p-6">
            <h3 className="text-xl font-bold text-gray-900 mb-4">Importar Inventario (JSON)</h3>
            <textarea
              value={bulkJson}
              onChange={(e) => setBulkJson(e.target.value)}
              className="w-full h-64 px-4 py-2 border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-transparent font-mono text-sm"
              placeholder='[{"id_elemento": "...", "nombre_elemento": "...", "tipo_inventario": "...", "distrito_id": 1}]'
            />
