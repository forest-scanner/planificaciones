...
import type { Programa, Actuacion, Distrito } from "@/shared/types";
...
export default function ProgramasPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const url = editingItem ? `/api/programas/${editingItem.id}` : "/api/programas";
    ...
    await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        id_actuacion: parseInt(formData.id_actuacion),
    ...
    setShowModal(false);
    setEditingItem(null);
    setFormData({ nombre_programa: "", id_actuacion: "", fecha_inicio: "", fecha_fin: "", distrito_id: "" });
    fetchItems();
  };
  ...
  const handleEdit = (item: any) => {
    ...
    setFormData({
      ...
      id_actuacion: item.id_actuacion?.toString() || "",
    ...
    <Layout>
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <h2 className="text-2xl font-bold text-gray-900">Programas</h2>
          <button
            onClick={() => {
              setEditingItem(null);
              setFormData({ nombre_programa: "", id_actuacion: "", fecha_inicio: "", fecha_fin: "", distrito_id: "" });
              ...
              <tbody className="divide-y divide-emerald-100">
                {items.map((item) => (
                  <tr key={item.id} className="hover:bg-emerald-50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm text-gray-600">{item.distrito_nombre || "-"}</td>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Actuación</label>
                <select
                  value={formData.id_actuacion}
                  onChange={(e) => setFormData({ ...formData, id_actuacion: e.target.value })}
                  className="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
                  required
                >
                  <option value="">Seleccionar actuación</option>
                  {actuaciones.map((a) => (
                    <option key={a.id} value={a.id}>{a.nombre_actuacion}</option>
                  ))}
                </select>
              </div>
              <div>
